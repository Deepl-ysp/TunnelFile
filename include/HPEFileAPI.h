...
namespace HPEFileAPI
{
    ...
    static bool starts_with(const std::vector<uint8_t> &data, const Signature &sig)
    {
    ...
    std::string FileMimeType(const std::vector<uint8_t> &data)
    {
        ...
        {
            if (sig.mime && starts_with(data, sig))
            {
                return sig.mime;
            }
        }
        if (data.size() >= 12 && starts_with(data, Signature{"\x52\x49\x46\x46", 4, nullptr}))
        {
