...
namespace HPENetWork
{
    ...
    public:
        ...
        TcpSession(asio::io_context &io, asio::ip::tcp::socket socket)
            : io_(io), socket_(std::move(socket)), heartbeat_timer_(io), strand_(io)
        {
        }
                                       ...
                                       if (!ec)
                                       {
                                           ...
                                           else
                                           {
                                               ...
                                               session->set_on_message(
                                                   [this, weak_s = TcpSession::WeakPtr(session)](TcpSession::Ptr, const std::string &data)
                                                   {
                                                       if (on_message_)
                                               ...
                                               session->set_on_disconnect(
                                                   [this, weak_s = TcpSession::WeakPtr(session)](TcpSession::Ptr)
                                                   {
                                                       std::lock_guard<std::mutex> lock(sessions_mutex_);
                                                       if (auto s = weak_s.lock())
                                                           sessions_.erase(s);
    ...
    public:
        ...
        TcpClient(asio::io_context &io, const std::string &host, uint16_t port)
            : io_(io), host_(host), port_(port), reconnect_timer_(io)
        {
        }
        ...
        void scheduleReconnect()
        {
            ...
            reconnect_timer_.async_wait(
                [this, self](asio::error_code ec)
                {
                    ...
                    if (next > NET_MAX_RETRY_INTERVAL)
                        next = NET_MAX_RETRY_INTERVAL;
